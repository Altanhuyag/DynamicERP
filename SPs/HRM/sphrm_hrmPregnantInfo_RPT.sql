IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sphrm_hrmPregnantInfo_RPT')
DROP PROC sphrm_hrmPregnantInfo_RPT
GO
CREATE PROC sphrm_hrmPregnantInfo_RPT
(
	@XML		  NVARCHAR(MAX),
...
select A.*,P.PositionName, year(GETDATE())-YEAR(E.Birthday) as Age, PF.ProfessionName, DI.DepartmentName,E.FirstName, E.LastName,F.ConfigValue as Position, H.ConfigValue as EmployeeName 
,((year(Getdate()) - Year(E.NowEnterPositionDate)) + isnull(S.HistoryYear,0)) WorkedYear from hrmEmployeeCause A
...
where A.CreatedFormName='frmPregnant' and case when @DepartmentPkID='' then '' else E.DepartmentPkID end = case when @DepartmentPkID='' then '' else @DepartmentPkID end
END
GO
