IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sphrm_EducationInfo_RPT')
DROP PROC sphrm_EducationInfo_RPT
GO
CREATE PROC sphrm_EducationInfo_RPT
(
	@XML		  NVARCHAR(MAX),
...
BEGIN
	...
	declare @EducationPkID nvarchar(16),
	@QueryString nvarchar(max),
	@String nvarchar(max),
	@i				int

	DECLARE db_cursor CURSOR FOR  
	SELECT EducationPkID FROM hrmEducationInfo 

	OPEN db_cursor   
	FETCH NEXT FROM db_cursor INTO @EducationPkID
	set @QueryString=''
	set @i=0
	WHILE @@FETCH_STATUS = 0   
	BEGIN   
		   set @i=@i+1 
		   SET @QueryString = @QueryString + 'sum(case when A.Gender=''0'' and ALI.EducationPkID='+@EducationPkID+' then 1 else 0 end) as F'+CONVERT(nvarchar,@i)+','
		   SET @QueryString = @QueryString + 'sum(case when ALI.EducationPkID='+@EducationPkID+' then 1 else 0 end) as S'+CONVERT(nvarchar,@i)+','
		   FETCH NEXT FROM db_cursor INTO @EducationPkID  
	END   
	...
	left join hrmEducationInfo ALI on A.EducationPkID=ALI.EducationPkID
